...
export default function GardenDesignPage() {
  ...
  const { service } = useService('garden', currentLanguage);
          ...
          <div>
            <div className="text-center mx-auto mb-8">
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold mb-6">
                <span className="text-gray-200">{service?.hero.title || t.servicePages?.garden?.hero?.title || 'Garden Design'}</span>
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
                {service?.hero.subtitle || t.servicePages?.garden?.hero?.subtitle || 'Beautiful garden design and landscaping services to enhance your outdoor space'}
              </p>
            </div>
          </div>
        </Parallax>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-4xl sm:text-5xl md:text-5xl lg:text-6xl leading-tight font-bold section-title mb-8">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-blue-200 to-cyan-300">
                  {service?.solutions.title || t.servicePages?.garden?.solutions?.title || 'Professional Solutions'}
                </span>
              </h2>
              <p className="text-gray-200 text-lg md:text-xl leading-relaxed mb-6 elegant-text">
                {service?.solutions.description1 || t.servicePages?.garden?.solutions?.description1 || 'Individual approach to design and landscaping. I create functional and aesthetic gardens tailored to your wishes and site conditions.'}
              </p>
              <p className="text-gray-200 text-lg md:text-xl leading-relaxed mb-6 elegant-text">
                {service?.solutions.description2 || t.servicePages?.garden?.solutions?.description2 || 'Selection of plants, lighting, and surfaces — for harmony and long service life.'}
              </p>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="text-center elegant-card p-6 animate-slide-up relative">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="text-gray-300">{service?.solutions.projectsCompleted || t.servicePages?.garden?.solutions?.projectsCompleted || 'Completed Projects'}</div>
                  </div>
                </div>
                <div className="text-center elegant-card p-6 animate-slide-up relative" style={{animationDelay: '0.1s'}}>
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="text-gray-300">{service?.solutions.yearsExperience || t.servicePages?.garden?.solutions?.yearsExperience || 'Years of Experience'}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="elegant-card p-8 animate-slide-up relative">
              <Meteors number={20} />
              <div className="relative z-10">
                <h3 className="text-2xl font-bold elegant-title mb-6">{service?.services.title || t.servicePages?.garden?.services?.title || 'Our Services'}</h3>
                <div className="space-y-4">
                  {(service?.services.items || t.servicePages?.garden?.services?.items || [
                    'Landscape design',
